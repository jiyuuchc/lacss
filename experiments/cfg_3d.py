...
def _train_data_map(
    ...
    *,
    ...
    data = augment.transpose(data, axes=axes, p=0.5)
    ...
    data = augment.flip_top_bottom(data, p=0.5)
...
def format_train_data(x):
    ...
    masks = tf.pad(
        x["masks"], [[0, padding], [0, 0], [0, 0], [0, 0]], constant_values=-1.0
    )

    return dict(
        image=x["image"], gt_locations=locs, image_mask=x["image_mask"] >= 0.5
    ), dict(gt_bboxes=bboxes, gt_masks=masks)
...
def format_test_data_3d(data, *, crop_size=CROP_SIZE):
    ...
    data = augment.random_crop_or_pad(
        data,
        target_size=(crop_size,) * 3,
        clip_boxes=False,
        area_ratio_threshold=0.5,
    )

    return dict(image=data["image"], image_mask=data["image_mask"] >= 0.5,), dict(
...
def get_config():
    ...
    config.data.ds_val = tfds.load("combined_3d", split="train").map(
        format_test_data_3d
    )
