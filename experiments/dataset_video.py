    ...
    def __getitem__(self, n):
        ...
        if segfn.exists():
            ...
            if label.max() == len(data["centroids"]):
                locs = np.array([rp.centroid for rp in regionprops(label)]) + 0.5
    ...
    for mov, rescale in all_movies:
        ...
        for frm in range(starting_frame, end_frame):
            ...
            if "bboxes" in data:
                ...
                label["gt_bboxes"] = np.pad(
                    data["bboxes"], [[0, padding], [0, 0]], constant_values=-1
                )
...
def ctmv_video_ds(datapath, n_refs=0, method=None):
    ...
    all_movies = [
        (CtmvVideo(datapath / name), 35 / sz) for name, sz in ctmv_catalog.items()
    ]
...
def ctmv_frame_ds(datapath, padding=128):
    ...
    def _format(data, padding, rescale):
        ...
        data["centroids"] = np.pad(
            data["centroids"], [[0, n_pad], [0, 0]], constant_values=-1
        )
        data["bboxes"] = np.pad(
            data["bboxes"], [[0, n_pad], [0, 0]], constant_values=-1
        )
