...
def segment(img, scale=100.0, sigma=0.8, min_size=20):
    img = img - np.mean(img, axis=(0, 1))
    img = img / 2 / (np.linalg.norm(img, axis=(0, 1)) / 512 + 1e-8) + 0.5
...
def _to_mask(img, pred):
    ...
    label = label.at[pred["instance_yc"], pred["instance_xc"]].max(
...
def train_data_gen_fn(teacher, data_path):
    ...
    for x, y, l in zip(X, Y, L):
        ...
        if tf.shape(data["locations"])[0] > 0:
            ...
            data = dict(
                image=data["image"], gt_locations=pad_to(data["locations"].numpy())
            ), dict(binary_mask=data["binary_mask"])
