...
def sample_prediction(data_path, model, rand8=_RAND_8):
    ...
    if preds["fg_pred"].shape[-1] > 0:
        ...
        preds["fg_pred"] = jnp.asarray(
            [x[..., t : t + 1] for x, t in zip(preds["fg_pred"], ts)]
        )
...
class SizeLoss(tx.Loss):
    ...
    def call(self, preds):
        valid_locs = (
            (preds["instance_yc"] >= 0)
            & (preds["instance_yc"] < 512)
            & (preds["instance_xc"] >= 0)
            & (preds["instance_xc"] < 512)
        )
        areas = (
            jnp.sum(
        ...
        loss = jnp.sum(loss, axis=1, where=preds["instance_mask"]) / (
            jnp.count_nonzero(preds["instance_mask"], axis=(1, 2, 3)) + 1e-8
        )
        return loss
