    ...
    for k in S:
        ...
        if supervised:
            ...
            data = (
                ...
                dict(binary_mask = augmented['binary_mask'].numpy(), tissue_type=np.asarray(_tissue_types[T[k]])),
            )
...
def val_dataset(data_path):
    ...
    for k in range(len(X)):
        ...
        data = (
            dict(image = image),
            dict(gt_boxes = pad_to(bboxes), gt_locations = pad_to(locs), tissue_type=np.asarray(_tissue_types[T[k]])),
            )
        data = jax.tree_util.tree_map(lambda v: np.expand_dims(v, 0), data)
