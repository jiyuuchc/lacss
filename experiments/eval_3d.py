...
def get_config(DATAPATH):
    ...
    config.data.organoid = dict(
        imgfiles=sorted(
            (DATAPATH / "Mouse-Organoid-Cells-CBG/train/images/").glob("*")
        )[:20],
        maskfiles=sorted(
            (DATAPATH / "Mouse-Organoid-Cells-CBG/train/masks/").glob("*")
        )[:20],
        scaling=[2.0, 0.33, 0.33],
    )
    config.data.platynereis = dict(
        imgfiles=sorted((DATAPATH / "Platynereis-Nuclei-CBG/train/images/").glob("*"))[
            :1
        ],
        maskfiles=sorted((DATAPATH / "Platynereis-Nuclei-CBG/train/masks/").glob("*"))[
            :1
        ],
        scaling=[3, 1, 1],
    )
    config.data.platynereis_ish = dict(
        imgfiles=sorted(
            (DATAPATH / "Platynereis-ISH-Nuclei-CBG/test/images/").glob("*")
        ),
        maskfiles=sorted(
            (DATAPATH / "Platynereis-ISH-Nuclei-CBG/test/masks/").glob("*")
        ),
...
def compute_ious(predictions, gt_label):
    ...
    mask_vols = (pred_masks >= 0.5).mean(
        axis=(1, 2, 3)
    ) * box_vols  # FIXME not exact value
...
def main(_):
    ...
    def run_eval(model, params):
        ...
        for name, data in config.data.items():
            ...
            for imgfile, maskfile in zip(data.imgfiles, data.maskfiles):
                ...
                if not img_mask.all():
                    centroids = np.floor(
                        predictions["pred_bboxes"].reshape(-1, 2, 3).mean(axis=1)
                    ).astype(int)
                ...
                mask_ious = mask_its / (
                    mask_vols[:, None] + gt_mask_vols - mask_its + 1e-8
                )
    ...
    if not cp.is_dir():
        ...
        if (cp / "model.pkl").exists():
            print(
                "seems to be a logging dir of multiple checkpoints. will eval all of them"
            )
