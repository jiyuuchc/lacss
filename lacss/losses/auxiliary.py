...
class AuxSizeLoss(Loss):
    ...
    def call(self, inputs, preds, **kwargs):
        ...
        valid_locs = (preds['instance_yc'] >= 0) & (preds['instance_yc'] < height) & (preds['instance_xc'] >= 0) & (preds['instance_xc'] < width)
        _, _, crop_size, _ = preds['instance_output'].shape
        areas = jnp.sum(preds['instance_output'], axis=(2,3), where=valid_locs, keepdims=True) / preds['instance_output'][0,0].size
        areas = jnp.clip(areas, EPS, 1.)
        loss =  self.a * jnp.clip(jax.lax.rsqrt(areas), 0., 1.e8)
        loss = jnp.sum(loss, axis=1, where=preds['instance_mask']) / (jnp.count_nonzero(preds['instance_mask'], axis=(1,2,3)) + EPS)
...
class AuxSegLoss(Loss):
    def call(self, inputs, preds, **kwargs):
        ...
        def _to_patch(img, yc, xc):
            ...
            img = jnp.pad(img, padding, constant_values=-1.)
            return img[yc+ps//2, xc+ps//2]

        @jax.vmap
        def _max_merge(pred):
            label = jnp.zeros([height + ps, width + ps]) - 1.e8
