...
from ..ops import coords_of_patches, sorbel_edges, sorbel_edges_3d
...
def _compute_edge(batch, prediction):
    ...
    combined_edges = combined_edges.at[:, zc, yc, xc].add(
        jnp.where(cmask, patch_edges, 0)
    )
    combined_edges = jnp.tanh(combined_edges)
...
def cks_boundry_loss(batch, prediction):
    ...
    if instance_edge_pred.ndim == 3:
        instance_edge_pred = instance_edge_pred[
            None,
        ]
