...
from ..ops import coords_of_patches, gather_patches, sub_pixel_samples
from .common import get_image_shape, mean_over_boolean_mask
...
def supervised_instance_loss(batch, prediction):
    ...
    if "gt_labels" in labels:  # labeled with image label
        ...
        assert (
            gt_segs.shape[0] == n_patches
        ), f"label length is {gt_segs.shape[0]}, but prediction length is {n_patches}"
        ...
        gt_patches = (
            jax.vmap(sub_pixel_samples)(
                gt_segs,
                jnp.stack([zc, yc, xc], axis=-1) - 0.5,
            )
            >= 0.5
        )
