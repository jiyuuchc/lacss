...
class AuxInstanceEdge(nn.Module):
    ...
    @nn.compact
    def __call__(self, x: jnp.ndarray, *, category:Optional[int] = None) -> jnp.ndarray:
        for n in self.conv_spec:
            x = nn.Conv(n, (3,3), use_bias=False)(x)
            x = nn.GroupNorm(num_groups=None, group_size=1, use_scale=False)(x)
            x = jax.nn.relu(x)
...
class AuxForeground(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray, *,  category:Optional[int] = None) -> jnp.ndarray:
        ...
        x = nn.GroupNorm(num_groups=n_ch, use_scale=False)(x)
        x = jax.nn.relu(x)
