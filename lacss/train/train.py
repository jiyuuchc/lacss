...
def _assign_label(module, gt_locs, decoded, config):
    ...
    indices = indices.at[idx].set(
        jnp.arange(N)[:, None]
    )  # this relies on Jax's oob behavior
    regr_mask = indices >= 0
...
def train_fn(
    ...
    assert (
        image.ndim in (3, 4) and image.shape[-1] == 3
    ), f"wrong img shape {image.shape}"
    assert gt_locations.ndim == 2 and gt_locations.shape[1] in (
        2,
        3,
    ), f"wrong location data shape {gt_locations.shape}"
    ...
    outputs["losses"] = _compute_losses(
        module, gt_locations, image_mask, outputs["detector"], config
    )
