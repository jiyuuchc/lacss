...
def _suppression_loop_body(inputs):
    ...
    box_slice = boxes[idx]
    similarity_func = box_iou_similarity if boxes.shape[-1]==4 else distance_similarity
    ...
    mask = jnp.arange(NMS_TILE_SIZE).reshape([1,-1]) > jnp.arange(NMS_TILE_SIZE).reshape([-1,1])
...
def sorted_non_max_suppression(
    ...
    def _trivial_suppress_all(inputs):
        idx, boxes, num_outputs, _, = inputs
    ...
    selected = jnp.argwhere((boxes >= 0).any(axis=-1), size=max_output_size, fill_value=-1).squeeze(-1)
