...
def locations_to_labels(locations, target_shape, threshold=1.5):
    ...
    mesh = jnp.mgrid[:height, :width] + .5  # [2, H, W]
    distances = flat_locations[:, :, None, None] - mesh  #[N, 2, H, W]
    distances_sq = (distances * distances).sum(axis=1)   #[N, H, W]

    # masking off invalid
    distances_sq = jnp.where(flat_locations[:, None, None, 0] >= 0, distances_sq, float('inf'))
    ...
    regression_target = jnp.take_along_axis(distances, indices, 0).squeeze(0) #[2, H, W]
