...
class MaskIndices:
    ...
    def gather(self, indices):
        ...
        new_data = jnp.concatenate([self.data[s:s+l, :2] for s, l in zip(sel_starts, sel_repeats)])
    ...
    def to_patches(self, y0x0, patch_size:int):
        ...
        patches = patches.at[shifted_data[:,2], shifted_data[:,0], shifted_data[:,0]].set(1)

        return patches

    def bboxes(self):
        min_y, min_x = jax.ops.segment_min(self.data[:, :2], self.data[:, -1]).transpose()
        max_y, max_x = jax.ops.segment_max(self.data[:, :2], self.data[:, -1]).transpose() + 1
