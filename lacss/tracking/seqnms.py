...
def compute_overlap_areas_given(boxes1, boxes2, area2):
    its = box_intersection(boxes1, boxes2)
    area1 = compute_area(boxes1)
    # area2 = compute_area(boxes2)
    iou = its / (area1[:, None] + area2 - its + 1e-8)
    return iou
...
def seq_nms(
    ...
    box_graph = build_box_sequences(
        boxes, scores, labels, linkage_threshold=linkage_threshold
    )
...
def build_box_sequences(boxes, scores, labels=[], linkage_threshold=0.5):
    ...
    for f in range(boxes.shape[0] - 1):
        ...
        if f == 0:
            areas_f = compute_area(
                boxes_f.astype(np.double)
            )  # (boxes_f[:,2] - boxes_f[:,0] + 1) * (boxes_f[:,3] - boxes_f[:,1] + 1)
        else:
            areas_f = areas_f1

        # calculate areas for boxes in next frame
        areas_f1 = compute_area(
            boxes_f1.astype(np.double)
        )  # (boxes_f1[:,2] - boxes_f1[:,0] + 1) * (boxes_f1[:,3] - boxes_f1[:,1] + 1)

        adjacency_matrix = []
        for i, box in enumerate(boxes_f):
            overlaps = compute_overlap_areas_given(
                np.expand_dims(box, axis=0).astype(np.double),
                boxes_f1.astype(np.double),
                areas_f1.astype(np.double),
            )[0]
            ...
            if len(labels) == 0:
                edges = [
                    ovr_idx
                    for ovr_idx, IoU in enumerate(overlaps)
                ...
                edges = [
                    ...
                    for ovr_idx, IoU in enumerate(overlaps)
                    if IoU >= linkage_threshold
                    and labels[f, i] == labels[f + 1, ovr_idx]
                ]
...
def find_best_sequence(box_graph, scores):
    ...
    for reverse_idx, frame_edges in enumerate(
        box_graph[::-1]
    ):  # list of edges between neigboring frames i.e frame dimension
        max_paths_f = []
        used_in_sequence = np.zeros(len(max_scores_paths[-1]), int)
        frame_idx = len(box_graph) - reverse_idx - 1
        for box_idx, box_edges in enumerate(
            frame_edges
        ):  # list of edges for each box in frame i.e. box dimension
            if (
                not box_edges
                ...
                prev_idx = np.argmax(
                    [max_scores_paths[-1][bidx][0] for bidx in box_edges]
                )
                ...
                max_paths_f.append(
                    (scores[frame_idx][box_idx] + score_so_far, path_so_far)
                )

        # create new sequence roots for boxes in frame at frame_idx + 1 that did not have links from boxes in frame_idx
        new_sequence_roots = [
            max_scores_paths[-1][idx]
            for idx, flag in enumerate(used_in_sequence)
            if flag == 0
        ]
...
def rescore_sequence(
    sequence, scores, sequence_frame_index, max_sum, score_metric="avg"
):
    ...
    if score_metric == "avg":
        ...
        max_score = 0.0
        for i, box_ind in enumerate(sequence):
            if scores[sequence_frame_index + i][box_ind] > max_score:
                max_score = scores[sequence_frame_index + i][box_ind]
        for i, box_ind in enumerate(sequence):
...
def delete_sequence(
    ...
    for i, box_idx in enumerate(sequence_to_delete):
        ...
        overlaps = compute_overlap_areas_given(
            np.expand_dims(seq_box, axis=0).astype(np.double),
            boxes[sequence_frame_index + i, :].astype(np.double),
            box_areas.astype(np.double),
        )[0]
        deletes = [
            ovr_idx for ovr_idx, IoU in enumerate(overlaps) if IoU >= suppress_threshold
        ]
        ...
        deletes.remove(box_idx)
        scores[sequence_frame_index + i, deletes] = -1
...
def _seq_nms(box_graph, boxes, scores, nms_threshold, score_metric="avg"):
    ...
    """
    while True:
        sequence_frame_index, best_sequence, best_score = find_best_sequence(
            box_graph, scores
        )
        ...
        delete_sequence(
            ...
            boxes,
            box_graph,
            suppress_threshold=nms_threshold,
        )
